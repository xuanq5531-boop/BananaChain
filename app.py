...
LANG_CONTENT = {
    'EN': {
        ...
        'app_sub': "Intelligent Banana Analysis System for Malaysian Agriculture & Supply Chain",
    ...
    'BM': {
        'app_title': 'BananaChain AI',
        'app_sub': "Sistem Analisis Pisang Pintar untuk Pertanian & Rantaian Bekalan Malaysia",
...
# ─────────────────────────────────────────────
# DISPLAY-ONLY LOGO CROPPING
# Removes transparent / near-uniform empty margins from logo assets.
# This changes presentation only; the original image files are untouched.
# ─────────────────────────────────────────────
def crop_logo_for_display(image_path, padding=10):
    try:
        ...
        if np.any(alpha < 250):
            ...
            foreground = np.max(np.abs(rgb - background), axis=2) > 22
...
with st.sidebar:
    if LOGO_PATH.exists():
        sidebar_logo = crop_logo_for_display(LOGO_PATH, padding=8)
        st.image(sidebar_logo, width=220)
    else:
        st.warning("Logo file not found: assets/logo.png")

    st.markdown("<div class='sidebar-brand'>BananaChain</div>", unsafe_allow_html=True)
    st.markdown("<div class='sidebar-tagline'>Chain the Future.</div>", unsafe_allow_html=True)
    st.divider()
...
with st.container():
    if TITLE_PATH.exists():
        title_logo = crop_logo_for_display(TITLE_PATH, padding=12)
        header_left, header_center, header_right = st.columns([1, 2.2, 1])
        with header_center:
            st.image(title_logo, use_container_width=True)
    else:
